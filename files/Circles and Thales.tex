...
Euclid's proof of this theorem is III.31.  Rather than using the sum of angles, he extends $AB$ external to the circle to $F$, forming $\angle FBC$.  Then $\angle ABC = \angle OAB + \angle OCB$ because $\triangle OAB$ and $\triangle OBC$ are both isosceles;  but $\angle FBC$ is external to $\triangle ABC$, so it is equal to the same two angles.  Therefore $\angle FBC = \angle ABC$.  It follows that both angles are right.  $\square$
...
\begin{center} \includegraphics [scale=0.3] {circle_constr.png} \end{center}
