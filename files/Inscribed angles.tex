...
This proof is short and sweet, but limited by the fact that we used the diameter for one of the arms of the inscribed angle.  

Here is a PWOW (proof without words) which does not have the diameter as one of the sides, but does span it.

\begin{center} \includegraphics [scale=0.30] {central_angle_PWOW} \end{center}

And here is a more general proof, with words
...
Euclid III.21  is an immediate consequence of the previous one, because the peripheral angle in III.20 is \emph{arbitrary}.  III.21 is a \emph{corollary} of III.20 --- it follows directly.  

Sometimes you see the term \emph{porism} in Euclid, which has a related meaning but refers to a proof which follows directly from an antecedent proof, rather than a theorem that is an easy consequence of a previous one.  (Don't ask me what the difference is, a theorem isn't much without the proof).

The two theorems are often considered to be the same.  III.20 might be better referred to as the central angle theorem, and III.21 as the inscribed angle theorem.  However, that is not the customary usage.

III.21 will appear often in the pages to come.  We will refer to it as \hyperref[sec:angles_on_same_arc]{\textbf{equal angles $\iff$ equal arcs}} or more simply, equal angles \emph{on} equal arcs, unless we slip, and call it the inscribed angle theorem.
...
But $\angle BD'C$ is external to $\triangle CDD'$ and $\angle BCD' + \angle DCD' = \angle BCD$.  Hence $\angle BCD' < \angle BCD$.

So $\angle BD'C > \angle BDC$ by sum of angles.

This is a contradiction.  A similar argument will show that $D$ cannot be internal to the circle.  Therefore, it must be that $D$ \emph{is on the circle}.
...
In $\triangle ABC$ draw the altitudes including $AD$ and $CF$ and find the orthocenter.  Draw the circumcircle and extend $CF$ to meet the circle at $X$.
...
But $\angle BAX$ cuts the same arc as $\angle DCF$.  Therefore the three angles marked with red dots are all equal.

$\triangle AFH \cong \triangle AFX$ (ASA).

It follows that $FX = FH$ and $\triangle BHX$ is isosceles as well with $\triangle ABH \cong \triangle ABX$ by SAS.
...
The second expression is the arithmetic mean of $a$ and $b$, while the first is the geometric mean.
...
Extend $AD$ to meet the circle at $X$, and also extend $AD$ vertically to $H \mid HD = DX$.  Join $HB$ and extend $CA$ to $E$.

Show that $CAE\perp HB$.

\begin{center} \includegraphics [scale=0.4] {inscribed_problem.png} \end{center}

A hint was given:  draw point $P$ where $HB$ cuts the circle.  Show that arc $PAC = $ arc $CX$.

Ignoring the hint for now, we are given that $BC$ is a perpendicular bisector of $HX$.  

Hence $\triangle HBX$ is isosceles with $BH = BX$ and $\angle BXD = \angle H$.

But $\angle BXA$ and $C$ cut the same arc $AB$.  So $H = C$.

We also have vertical angles so it follows that $\triangle AEH \sim \triangle ADC$.  Thus $\angle AEH$ is right.  

Alternatively $\triangle CEB \sim \triangle HDB$ and the result follows.
...
$H$ is shared.  It follows that $\triangle HAE \sim \triangle EBC$.  Since $BPEH$ are collinear, and the angles at $E$ are equal, they are both right.
