...
Since this immediately follows the first construction, it seems likely we will need an equilateral triangle.  So draw $\triangle ABD$ using EI.2.

The other thing we know how to do is to draw circles.  Construct a circle of radius $BC$ on center $B$.  Where that intersects the extension of $DB$ at $F$ we have a length equal to one side of the $\triangle$ plus $BC$.
...
We obtain $AG$ by subtraction. $AG = BF = BC$, as required.

If it is desired to draw a line segment of length $BC$ in some other direction from $A$, we just need another circle, centered at $A$.  That is I.3.

To summarize, we can mark off any length from one line segment onto another, even with just a collapsing compass.

\subsection*{construct a given angle at a given point}

Construct the triangle containing the desired angle by using SSS.

This can be done by transferring lengths as described above.  Suppose $\triangle ABC$ has the given angle as $\angle CAB$.  Transfer length $AB$ to the given point.  For convenience we refer to that point as $A$.  

On center $A$, transfer length $AC$ and use that to draw the circle of radius $AC$.  Do the same with $BC$ on center $B$.  The desired point $C$ is at the intersection of the two circles.
...
Then, erect the bisector of the black line at each point and extend the bisector to the target red line.
