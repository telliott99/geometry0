...
Here we look at the problem of angle bisection, dividing an angle into two equal parts.

We also look at  erecting a line perpendicular to a line at any point  

We extend that to draw the perpendicular line that cuts halfway between two end points (the perpendicular \emph{bisector}), or third, a perpendicular that passes through any point not on the line.  

We will show that if a point is on the perpendicular bisector, then it is equidistant from the two points used to construct the perpendicular.

We also prove the converse theorem, that \emph{every} point which is equidistant from the two end points lies on the bisector.
...
The tools we have are a straight-edge and a compass.  The compass is collapsible, meaning that it cannot be used to transfer distances since it loses its setting when lifted from the page.  

This is a limitation Euclid solves in the second and third propositions of book I, so it never really matters, except to fulfill Euclid's ideal of making the \emph{least possible assumptions}.  A non-collapsible compass was a luxury he didn't need, since he could accomplish the same end without it.

It's also important that the straight-edge is not a ruler, there is no way to measure distance by reference to marks on it.
...
$AF$ is shared, so $\triangle ADF \cong \triangle AEF$ by SSS, Euclid I.8.  If we would prefer a proof independent of SSS, things become awkward.  We have the three sides but no angles.  Later we will show that SSS implies SAS and we know SAS gives congruence.  So there is really no problem with the proof,

Assuming SSS is valid, then $\angle BAF$ is congruent to $\angle CAF$ and the given angle $\angle BAC$ is bisected.
...
Aiming for a contradiction, suppose that $P$ is equidistant from $B$ and $C$ but does not lie on the perpendicular bisector.  

It does not matter which side we choose.  Let $P$ lie on the same side of the bisector as $B$.
...
In the figure below, given that $AC = AB$ and $B = C$.
...
The angles labeled with red dots are equal by alternate interior angles and then vertical angles, while the angles labeled blue are equal by alternate interior angles.

But we are given that this triangle is isosceles, so blue and red are equal.  Therefore the exterior angle at $C$ is bisected by the horizontal.
