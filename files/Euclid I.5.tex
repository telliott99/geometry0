...
In modern usage, we call the method SAS or \emph{side angle side}.  Given:

$\circ$ \ $AB = DE$

$\circ$ \ $\angle BAC = \angle EDF$

$\circ$ \ $AC = DF$

It follows that the two triangles are congruent:  all three angles and all three sides are equal.
...
\begin{center} \includegraphics [scale=0.35] {EI_5f.png} \end{center}

Given $AB = AC$.  We will prove that the base angles are equal:  $\angle ABC = \angle ACB$.

The images below show two copies of the triangle, to compare congruent triangles formed within the \emph{same} figure.

\emph{Proof}.

Extend $AB$ and $AC$ so that $AD = AE$.  
\begin{center} \includegraphics [scale=0.35] {EI_5g.png} \end{center}
...
\begin{center} \includegraphics [scale=0.35] {EI_5h.png} \end{center}

It follows that $\angle DBC = \angle ECB$.  

And then, by supplementary angles (or subtraction), $\angle ABC = \angle ACB$.

$\square$

The original figure is extended to provide auxiliary shapes helpful in the proof.  This is a common theme.
...
We proved the converse of I.5 previously based on angle bisection.  Now that we have proved Euclid I.5, which provides the basis (through SSS) for bisection, this is logically solid.  Nevertheless, for completeness, here is Euclid's proof of I.6.  

It is the first \emph{Proof by Contradiction}.

\begin{center} \includegraphics [scale=0.16] {Euclid_I_6.png} \end{center}

\emph{Proof}.

Given $\angle ABC = \angle ACB$.  Aiming for a contradiction, suppose $AB \ne AC$.  

Then let one be less, say $AC$.  So cut off from $AB$ the length $BD = AC$.
...
Proof by contradiction is used quite often in \emph{Elements}.
