...
Probably the most fundamental idea concerning triangles is how to decide whether two triangles are \emph{congruent}.  

If two triangles have all 3 pairs of angles with the same measure, and the corresponding sides (opposite each angle) have the same length, then they are congruent.

However this requires more knowledge than what we usually have. 

\begin{center} \includegraphics [scale=0.3] {congruent1c.png} \end{center}

We would like to be able to say that \emph{if} two triangles share only some of these properties, \emph{then} the two triangles are congruent.  For example, if they have two sides the same, and the included angle is also the same, then we can prove that they are congruent, and thus, for example, the third side is also equal.

We will find several tests that, if passed, imply congruence.

In comparing two triangles, we allow one triangle to be rotated with respect to the other.  $\triangle ABC$ and $\triangle DEF$ are an example of this.  Equal sides have been colored to show which ones correspond.
...
Perhaps a practical definition would be that if you used a pair of scissors to cut out one triangle and then lay it on top of the second one, and find that it superimposes exactly, they would be congruent.

In fact this is how Euclid handles the issue of congruence in the first theorem of \emph{Elements}, I.4.  (It's the first theorem because the first three \emph{propositions} are actually constructions).  I.4 relates to a second test for congruence, called side-angle-side (SAS) which we will see in a minute.

A little demonstration may convince you that if the side lengths are equal, two triangles are congruent.  Draw one side of a triangle, and from its endpoints draw circles with radii the length of the other two sides.
...
We see that the radii cross at just two different points, which have mirror image symmetry above and below the original base.  Three given side lengths can only be drawn together to give two resulting triangles, and these two shapes are mirror images.  
...
On a cautionary note, it may happen that a particular diagram incorporates an unspoken \emph{assumption}.  We will have to be careful about that and return to the issue later.
...
If $b$ is longer than this, there are two points $A$ and $A'$ where $b$ can intersect with the side projecting from vertex $B$. 

This is called the \emph{ambiguous case}:  we cannot say whether they are congruent or not without more information.  It is not that we don't know anything, we just don't know enough to choose one of two possibilities.

It matters what kind of angle we have at $B$.  If $B$ is a right angle, then the points $A$ and $A'$ lie on either side of $B$, and it turns out that they are congruent mirror images.  The right angle case is not ambiguous, and we'll say more in the chapter on right triangles.

If $B$ is obtuse, then the other angles in the triangle must be acute, and then there is no ambiguity either.

\begin{center} \includegraphics [scale=0.2] {ambig.png} \end{center}

Finally, it turns out that the two possibilities for the angle at $A$ are related because $\triangle CAA'$ is isosceles.  This has the consequence that the two possible triangles for a case of SSA have either an acute angle at $A'$ or its supplement at $A$  If we can show that in addition to SSA we know that two candidates for congruence also have both acute or both obtuse angles at this vertex then they \emph{are} congruent.
...
When we work with similar triangles and their corresponding angles, we will strive to label the angles in the order of their equality.  Thus if $\triangle ABC \sim \triangle QPR$, you should expect that $A = Q$, and so on.

\subsection*{proofs or axioms}
...
It won't hurt if you think of them as axioms.  The famous mathematician David Hilbert did this.  We will revisit the issue when we look at Euclid's proofs.
