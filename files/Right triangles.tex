...
There is only one angle where the hypotenuse will terminate on the vertical extension from the right angle (well technically there are two, but the other is the mirror image).
...
Let $\triangle ABC$ be a right triangle, with a right angle at $C$.  We know that the base angles add up to a right angle.
\[ \angle CAB + \angle CBA = 90 \]

When we draw the perpendicular to the hypotenuse that goes through the upper vertex, that is an \emph{altitude} of the triangle.  We obtain two smaller right triangles.  Because of the right angle, these smaller triangles are similar to each other and to the original.

Thus
\[ \angle CBA + \angle DCB = 90 \]
It follows that
\[ \angle CAB =  \angle DCB \]
...
Because it has four right angles at its vertices, $EMDC$ is a rectangle.  (The fourth, at $M$, follows by sum of angles, or because $MD$ and $ME$ are individually perpendicular to lines that are themselves perpendicular, at $C$).
...
There is a simpler and more elegant proof of the above theorem.  One uses Thales' theorem, which we will cover again later, but mentioned at the beginning of the book.  

Any right triangle can be placed in (inscribed into) a circle, with the hypotenuse as the diameter.  

Since $AM$ and $BM$ are equal, $M$ is the center and they are radii, so $CM$ is also a radius of this circle.

Another proof uses similar triangles.  However, we have not established that theory yet, so we will skip it for now.

We can also give a proof by contradiction.  But first we need another of Euclid's theorems.
...
In $\triangle ABC$, suppose that $BC > AC$.  We will show that $A > B$.

\emph{Proof}.

Mark off $CD = AC$.  

By the isosceles triangle theorem, $\angle CAD = \angle CDA$.

$\angle CAD < A$.

But $\angle CDA$ is the external angle for $\triangle ABD$ so it is greater than $\angle B$.

We have $A > \angle CAD$, $\angle CAD = \angle CDA$ and $\angle CDA > B$.

By transitivity of inequality:  $A > B$. 

$\square$

Since $AC > AB$, the angle opposite $AC$ is greater than the angle opposite $AB$.

Now, proceed to label $\angle MCA$ as $\alpha$ and $\angle MCB$ as $\beta$.

\begin{center} \includegraphics [scale=0.20] {rt_tri3.png} \end{center}

\emph{Proof}.

Aiming for a contradiction, let $MC$ not be equal to $AM$ and $BM$.  Suppose it is less.

Then by I.18, $\alpha < A$ and $\beta < B$.  But the sum of the right-hand sides is a right angle, so $\alpha + \beta < 90$.

But $\alpha + \beta$ is a right angle.  This is a contradiction.  

Hence $MC$ is not less than $AM$ and $BM$.

The other part of the proof is exactly the same, with $>$ rather than $<$.
...
Above we had the result that the vertical line to the hypotenuse (called an \emph{altitude}), forms two smaller right triangles similar to the first.  One can construct equal ratios and use them to make a proof of the Pythagorean theorem.  This is left as an exercise, but we'll return to it when we have laid out the basic theory of similarity.
