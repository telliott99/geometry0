...
We will prove this for all triangles when we deal with the theory of similarity more explicitly.  Euclid VI.2 accomplishes this elegantly.
...
Any triangle can be cut into two right triangles by drawing its \emph{altitude}, $h$, where $h$ is perpendicular to $c$.  (For an obtuse or right triangle, there is only one vertex where the altitude is internal, and hence achieves this).
...
\begin{center} \includegraphics [scale=0.25] {newton4.png} \end{center}

In $\triangle ABC$, draw the altitude from the vertex $C$ down to meet the base in a right angle at $F$.  Let it have length $H$.

This altitude divides the base into lengths $x$ and $y$.  Now draw a second altitude from vertex $A$ to the side opposite at $D$.  

What is the height $h$ above the base where the two lines cross?  

It turns out that $h$ depends only on $x$, $y$ and $H$.

It follows that the altitude $BE$ crosses $CF$ at the same height $h$ above $AB$.  So the three altitudes cross at the same point.

\emph{Proof}.

Since we are using $H$ for height, let the orthocenter where the altitudes cross be $X$.  

 $\triangle AFX \sim \triangle CDX$, since they are both right triangles and share vertical angles.  This means that the complementary angles marked with magenta dots are equal.
 
 But $\triangle CFB$ is also similar as well, because it is a right triangle containing the marked angle.
 
\begin{center} \includegraphics [scale=0.25] {newton4.png} \end{center}

In $\triangle AFX$ the side opposite the marked angle ($\angle BAD$) has length $h$ so the ratio of sides is $h/x$.

In $\triangle CFB$, the side opposite the marked angle ($\angle BCF$) has length $y$, so the ratio of sides is $y/H$.  We have
