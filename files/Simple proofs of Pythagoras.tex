...
The tablet \textbf{Plimpton 322} contains (by extrapolation) the triplet 4601-4800-6649 and it dates to about 1800 BCE.  Maor analyzes this in his book on the theorem of Pythagoras.  

Row 3 of the tablet contains the numbers $1,16,41$ and $1,50,49$ which are in base 60 notation.  Thus $1 \cdot 3600 + 16 \cdot 60 + 41 = 4601$ and $1 \cdot 3600 + 50 \cdot 60 + 49 = 6649$.

\begin{center} \includegraphics [scale=0.4] {plimpton_trip.png} \end{center}

The third number of the triple isn't given but it's obvious since $6649^2 - 4601^2 = 23040000$, which is $4800^2$ or $1 \cdot 3600 + 20 \cdot 60$.

[ What is given is 55 07 41 15 33 45 --- I'm not quite clear on what that means.

\url{https://en.wikipedia.org/wiki/Plimpton_322}

It seems highly unlikely that these were found by searching randomly among squares.  It is believed that since $4800^2$ has $2 \cdot 3 \cdot 5$ as a factor that is a clue to the method of finding them.  They have other properties which suggest things for scholars to argue about. ]

There is even a triple with 5 decimal digits on the tablet.

One should not think that Pythagoras's theorem \emph{only} applies to triangles with integer side lengths.  For example, it applies to the isosceles right triangle with side length equal to $1$, whose hypotenuse is the real number $\sqrt{2}$.  Also, any integer solution can be modified.  For example
