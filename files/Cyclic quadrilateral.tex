...
\begin{center} \includegraphics [scale=0.3] {cq_conv} \end{center}

Let $\triangle ABC$ lie on a circle with $B = \theta$, subtended by arc $AC$.

If another point $D$ is subtended by the same arc, and has the same measure, it lies on the same circle.

\emph{Proof}.

Aiming for a contradiction, suppose $D$ has measure $\theta$ and intercepts the arc but does not lie on the circle.  Suppose it is external.

By Euclid III.21, any point lying on the circle and intercepting the same arc also has measure $\theta$.  Let $E$ on $AD$ be such a point with $\angle DCE = \alpha$.

But then, by the external angle theorem, we have $\theta + \alpha = \theta$.

This is absurd.

There cannot be such a point $D$.

A similar argument shows $D$ cannot be inside the circle.  Simply find $E$ by extending $AD$ to the circle.

$\square$

\subsection*{bisected angles and SSA}

Take an arbitrary triangle and draw its circumcircle.  Bisect one angle, say $A$, and find where the bisector cuts the circle at $D$.
